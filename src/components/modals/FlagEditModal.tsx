        ...
        <div className="fixed inset-0 bg-black/80 z-[70] flex items-center justify-center p-2 backdrop-blur-sm touch-none">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl flex flex-col max-h-[95vh] overflow-hidden">
                ...
                <div className="flex-1 overflow-auto bg-gray-900 flex items-center justify-center relative touch-none p-4 min-h-[50vh]">
                    <div ref={situationImageRef} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerLeave={handlePointerUp} className="relative inline-block select-none touch-none shadow-2xl border-2 border-gray-700">
                        <img src={photoPreviewUrl} alt="Situation large" style={{ maxHeight: '60vh' }} className="w-auto block pointer-events-none" />
                        {showSituationFlag && (
                            <div onPointerDown={handlePointerDown} style={{ left: `${situationFlagPos.x}%`, top: `${situationFlagPos.y}%`, width: `${flagSize}px`, transform: 'translate(-50%, -100%)' }} className="absolute cursor-grab active:cursor-grabbing z-20 drop-shadow-xl touch-none" title="Glissez pour positionner le drapeau">
